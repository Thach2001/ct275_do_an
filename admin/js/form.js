$_DOMAIN = 'http://localhost/ct275_do_an/admin/';
...
$('#formEditPost button').on('click', function() {
    ...
    $descr_edit_post = $('#descr_edit_post').val();
...
$('#formAddAcc button').on('click', function() {
    ...
    if ($un_add_acc == '' || $pw_add_acc == '' || $repw_add_acc == '' || $email_add_acc == '') {
...
$('#formRegister button').on('click', function() {
    ...
    $repass_register = $('#repass_register').val();

    if ($user_register == '' || $email_register == '' || $pass_register == '' || $repass_register == '') {
        $('#formRegister .alert').removeClass('hidden');
        $('#formRegister .alert').html('Vui lòng điền đầy đủ thông tin.');

    } else {
        $.ajax({
            url: $_DOMAIN + 'register.php',
            type: 'POST',
            data: {
                user_register: $user_register,
                email_register: $email_register,
                pass_register: $pass_register,
                repass_register: $repass_register,
                action: 'add_reg'
            },
            success: function(data) {
                $('#formRegister .alert').html(data);
            },
            error: function() {
                $('#formRegister .alert').removeClass('hidden');
                $('#formRegister .alert').html('Đã có lỗi xảy ra, hãy thử lại.');
            }
        });
    }
});
...
// Mở khóa tài khoản chỉ định trong bảng danh sách
